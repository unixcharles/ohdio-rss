...
RSpec.describe SyncShowJob, type: :job do
  it "enqueues page jobs using highest max_episodes across feeds" do
    Feed.create!(name: "Feed A", show_id: 123, max_episodes: 50)
    Feed.create!(name: "Feed B", show_id: 123, max_episodes: 80)
    clear_enqueued_jobs

    show_record = instance_double(Show, ohdio_id: 123, ohdio_type: "balado")
    fetched_show = instance_double(Ohdio::Show, page_size: 20, total_episodes: 500)
    service = instance_double(OhdioShowSyncService, call: [ show_record, fetched_show ])

    expect(OhdioShowSyncService).to receive(:new).with(show_id: 123, max_episodes: 80).and_return(service)
  ...
  it "caps effective max_episodes at 1000" do
    Feed.create!(name: "Feed A", show_id: 456, max_episodes: 1000)
    clear_enqueued_jobs

    show_record = instance_double(Show, ohdio_id: 456, ohdio_type: "balado")
    fetched_show = instance_double(Ohdio::Show, page_size: 25, total_episodes: 5000)
    service = instance_double(OhdioShowSyncService, call: [ show_record, fetched_show ])

    expect(OhdioShowSyncService).to receive(:new).with(show_id: 456, max_episodes: 1000).and_return(service)
