...
RSpec.describe ResolveAudioContentJob, type: :job do
  before do
  ...
  it "registers global cooldown on non-429 Ohdio errors" do
    audio_content = AudioContent.create!(external_id: "media-1", resolved: false)
    ...
    described_class.perform_now(audio_content.id)

    expect(OhdioApiThrottle).to have_received(:register_error!)
    expect(audio_content.reload.resolve_error).to include("HTTP 500")
  end

  it "registers global cooldown on 429 Ohdio errors" do
    audio_content = AudioContent.create!(external_id: "media-2", resolved: false)
    client = instance_double(Ohdio::Client)
    ...
    expect(OhdioApiThrottle).to have_received(:register_error!)
    expect(audio_content.reload.resolve_error).to include("HTTP 429")
  end
end
