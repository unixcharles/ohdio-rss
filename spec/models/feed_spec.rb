...
RSpec.describe Feed, type: :model do
  ...
  it 'is valid with name and show_external_id' do
    feed = described_class.new(name: 'My Feed', show_external_id: 123)

    expect(feed).to be_valid
  end

  it 'is invalid without a name' do
    feed = described_class.new(name: nil, show_external_id: 123)

    expect(feed).not_to be_valid
    expect(feed.errors[:name]).to include("can't be blank")
  end

  it 'is invalid without a show_external_id' do
    feed = described_class.new(name: 'My Feed', show_external_id: nil)

    expect(feed).not_to be_valid
    expect(feed.errors[:show_external_id]).to include("can't be blank")
  end

  it 'is invalid with a non-integer show_external_id' do
    feed = described_class.new(name: 'My Feed', show_external_id: 'show-123')

    expect(feed).not_to be_valid
    expect(feed.errors[:show_external_id]).to include('is not a number')
  end

  it 'allows duplicate show_external_id values' do
    described_class.create!(name: 'Existing Feed', show_external_id: 123)
    duplicate_feed = described_class.new(name: 'Duplicate Feed', show_external_id: 123)

    expect(duplicate_feed).to be_valid
  end

  it 'generates a uid on create' do
    feed = described_class.create!(name: 'My Feed', show_external_id: 123)

    expect(feed.uid).to be_present
  end

  it 'defaults exclude_replays to true' do
    feed = described_class.create!(name: 'My Feed', show_external_id: 124)

    expect(feed.exclude_replays).to be(true)
  end

  it 'defaults max_episodes to 100' do
    feed = described_class.create!(name: 'My Feed', show_external_id: 125)

    expect(feed.max_episodes).to eq(100)
  end

  it 'is invalid when max_episodes is above 1000' do
    feed = described_class.new(name: 'My Feed', show_external_id: 126, max_episodes: 1001)
  ...
  describe '#show' do
    it 'maps feed.show_external_id to show.external_id' do
      show = Show.create!(external_id: 123, title: 'Fetched Show')
      feed = described_class.create!(name: 'My Feed', show_external_id: 123)
  ...
  describe '#filtered_episodes' do
    ...
    it 'filters out emission episodes without valid segments' do
      show = Show.create!(external_id: 889, title: 'Emission Show', ohdio_type: 'emission_premiere')
      feed = described_class.create!(name: 'My Feed', show_external_id: 889)

      show.episodes.create!(ohdio_episode_id: 'ep-1', title: 'Valid', has_valid_segments: true)
      show.episodes.create!(ohdio_episode_id: 'ep-2', title: 'Invalid', has_valid_segments: false)
  ...
  describe '#filtered_segments_for_episode' do
    it 'returns all valid segments when segment_query is blank' do
      ...
      episode.segments.create!(title: 'politique', audio_content_external_id: 'm1', seek_time: 0, duration: 20, position: 1)
      episode.segments.create!(title: 'invalide', audio_content_external_id: nil, seek_time: 10, duration: 20, position: 2)
    ...
    it 'filters valid segments using segment_query' do
      ...
      episode.segments.create!(title: 'bloc culture', audio_content_external_id: 'm2', seek_time: 10, duration: 20, position: 2)
  ...
  describe 'sync enqueue callbacks' do
    it 'enqueues a sync on create' do
      described_class.create!(name: 'My Feed', show_external_id: 123)

      expect(FeedRefreshScheduler).to have_received(:enqueue).with(123, force: true)
    end

    it 'enqueues a sync when show_external_id changes' do
      feed = described_class.create!(name: 'My Feed', show_external_id: 123)
      allow(FeedRefreshScheduler).to receive(:enqueue)

      feed.update!(show_external_id: 456)

      expect(FeedRefreshScheduler).to have_received(:enqueue).with(456, force: true)
    end

    it 'does not enqueue when show_external_id is unchanged' do
      feed = described_class.create!(name: 'My Feed', show_external_id: 123)
