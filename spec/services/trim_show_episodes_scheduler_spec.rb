...
RSpec.describe TrimShowEpisodesScheduler do
  before do
    Rails.cache.clear
  ...
  it "enqueues only once during throttle window" do
    ...
    expect(enqueued_jobs.count { |job| job[:job] == TrimShowEpisodesJob }).to eq(1)
  end
  ...
  it "enqueues immediately when forced" do
    ...
    expect(enqueued_jobs.count { |job| job[:job] == TrimShowEpisodesJob }).to eq(2)
  end
end
