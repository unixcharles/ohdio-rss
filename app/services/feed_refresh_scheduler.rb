class FeedRefreshScheduler
  ...
  def self.enqueue(show_external_id, force: false)
    return if show_external_id.blank?

    if force || should_enqueue?(show_external_id)
  ...
  def self.should_enqueue?(show_external_id)
    last_enqueued_at = @memory_last_enqueued_at[show_external_id.to_s]
    return false if last_enqueued_at && (Time.current - last_enqueued_at) < ENQUEUE_TTL

    key = cache_key(show_external_id)

    write_result = Rails.cache.write(key, true, expires_in: ENQUEUE_TTL, unless_exist: true)
    if write_result
      @memory_last_enqueued_at[show_external_id.to_s] = Time.current
      return true
    end
  ...
  def self.cache_key(show_external_id)
    [ "show-refresh-enqueued", show_external_id.to_s ]
  end
